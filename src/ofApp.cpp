...
void ofApp::setup(){
	...
	//update the SVGs
	galeriaIconos["corazon"].load("corazon.svg");
	galeriaIconos["conejo"].load("conejo.svg");
	galeriaIconos["gafas"].load("gafas.svg");
	galeriaIconos["estrella"].load("estrella.svg");
	galeriaIconos["gusano"].load("gusano.svg");
	galeriaIconos["rock"].load("rock.svg");
	galeriaIconos["alas"].load("alas.svg"); // Para el rezo
	galeriaIconos["dormir"].load("dormir.svg");
	galeriaIconos["diablo"].load("diablo.svg");
	galeriaIconos["fuego"].load("fuego.svg");
	galeriaIconos["pez"].load("pez.svg");
	galeriaIconos["burbuja"].load("burbuja.svg");
	galeriaIconos["pulgar"].load("pulgar.svg");
}
		...
		for (int i = 0; i < numManos; i++) {
			...
			// Pulgar lejos del índice y resto de dedos cerrados
				...
				if (hand[4].y < hand[2].y) {
					...
					lanzarFigura("pulgar", puntaIndice);
				}
			}
			...
			if (isHandClosed) {
				...
				// if (isHandHorizontal(hand)) {
				lanzarFigura("burbuja", puntaIndice);
				// }
			}
...
void ofApp::lanzarFigura(string nombreGesto, ofPoint posicion) {
	float now = ofGetElapsedTimef();
	...
	if (now - lastSpawnTime > 0.5) {
		...
		if (galeriaIconos.count(nombreGesto)) {
			...
			//Aprecen un poco random alreedor de la mano
			f.pos.x += ofRandom(-20, 20);
			f.pos.y += ofRandom(-20, 20);
